...
namespace fun 
{
  ...
  class conic3 : private matrix3<_K>
    {
      /// Value typedef.
    ...
    public:
      /// Default constructor.
      conic3(const _K& a, 
	    const _K& b, const _K& c, 
	    const _K& d, const _K& e, const _K& f) 
        : _Base(a, b, d,
		b, c, e,
		d, e, f) { }

      /// Construct from a matrix @a A
      conic3(const matrix3<_K>& M) : _Base(M + M.transpose()) { }

      // Lets the compiler synthesize the copy constructor
      // conic3 (const conic3<_K>&);
      /// Copy constructor
      template<typename _Up>
      conic3(const conic3<_Up>& C) : _Base(C.base()) { }

      /// Return the base class
      constexpr matrix3<_K> A() { return *this; }
      ...
      constexpr _K a() { return _Base::a(); }

      /// Return second element of conic3.
      constexpr _K b() { return _Base::d(); }

      /// Return third element of conic3.
      constexpr _K c() { return _Base::e(); }

      /// Return first element of conic3.
      constexpr _K d() { return _Base::g(); }

      /// Return second element of conic3.
      constexpr _K e() { return _Base::h(); }

      /// Return third element of conic3.
      constexpr _K f() { return _Base::i(); }

      // Lets the compiler synthesize the assignment operator
      // conic3<_K>& operator= (const conic3<_K>&);    

      line3<_K> polar(const point3<_K>& p) const
      { return A() * p; }

      /// Return true if a point @a p incident with this conic3
      bool incident(const point3<_K>& p) const
  ...
  inline bool
  operator==(const conic3<_K>& C1, const conic3<_K>& C2)
  { return C1.A() == C2.A(); }
  ...
  operator<<(std::basic_ostream<_CharT, _Traits>& os, const conic3<_K>& l)
  {
